...
function initialiseView({scrollToHash=true}={}){
 ...
 for(const viewport of [main.querySelector('[data-citymap]')]){
  ...
  const orgPins={};
  const SUFFIX_LABEL={nan:zh?'南山':'Nanshan',hhb:zh?'红磡湾':'Hung Hom Bay',west:zh?'西九龙':'West Kowloon',ytm:zh?'油尖旺':'Yau Tsim Mong'};
  for(const o of orgs){
  ...
  const updatePanel=()=>{
   ...
   writePanel(title,zh?'点击机构圆点，查看该机构与相关经历。':'Tap a pin to see that institution and its experience.',zh?'地区':'Region');
  };
