...
// The same Natural Earth coastline works offline and in both colour themes.
const coastRings=(mapGeometry.overviewPath.match(/M[^M]+/g)||[]).map(path=>{
 const points=path.slice(1,-1).split('L').map(pair=>pair.split(',').map(Number));
 return [...points,points[0]].map(([x,y])=>[103+x*22/300,43-y*24/330]);
});
const mapLand={type:'FeatureCollection',features:[{type:'Feature',geometry:{type:'MultiPolygon',coordinates:coastRings.map(ring=>[ring])},properties:{}}]};
await writeFile(join(out,'assets/map-land.mjs'),`export default ${JSON.stringify(mapLand)};`);
...
for(const lang of ['zh','en']){
 ...
 const contactPanel=`<aside id="contact-panel" class="contact-panel" popover aria-labelledby="contact-title"><header><div><span class="eyebrow">TIN-YEH HUANG</span><h2 id="contact-title">${t('联系','Contact')}</h2></div><button popovertarget="contact-panel" popovertargetaction="hide" aria-label="${t('关闭联系面板','Close contact panel')}">${icon('x')}</button></header><div class="contact-lead"><span class="contact-logo contact-logo-mail" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="5" width="18" height="14" rx="3"/><path d="m4 7 8 6 8-6"/></svg></span><div><small>${t('直接邮件联系','Email')}</small><a href="mailto:tin-yeh.huang@connect.polyu.hk"><span>tin-yeh.huang</span><span>@connect.polyu.hk</span></a></div><button class="copy-email" data-copy-email aria-label="${t('复制邮箱','Copy email')}">${icon('copy')}</button></div><span data-copy-status role="status"></span><div class="contact-profiles"><a href="https://www.linkedin.com/in/tin-yeh-huang-59bba3289/" target="_blank" rel="noopener noreferrer"><span class="contact-logo contact-logo-linkedin" aria-hidden="true">in</span><span>LinkedIn</span>${icon('arrow-up-right')}</a><a href="https://github.com/Huangty5121" target="_blank" rel="noopener noreferrer"><span class="contact-logo contact-logo-github" aria-hidden="true"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.111.82-.261.82-.577 0-.285-.011-1.234-.017-2.421-3.338.726-4.043-1.416-4.043-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.09-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.835 2.807 1.305 3.492.998.108-.775.418-1.305.762-1.605-2.665-.304-5.467-1.333-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.536-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.535 11.535 0 0 1 12 6.217c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.655 1.652.243 2.873.12 3.176.77.84 1.235 1.91 1.235 3.221 0 4.61-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222 0 1.606-.014 2.898-.014 3.293 0 .319.216.694.825.576C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg></span><span>GitHub</span>${icon('arrow-up-right')}</a></div></aside>`;
 const views=createViews({lang,prefix,h,t,icon,a});
 for(const [file,page] of specs){
  const active=page==='writingModern'?'notes':['engineering','social'].includes(page)||page.startsWith('read-')||page.startsWith('work-')?'collection':page;
  const title=page==='home'?'Tin-Yeh Huang':page==='engineering'?'NineToothed':page==='social'?t('社会创新','Social innovation'):page==='writingModern'?t('工业设计与中国现代化','Industrial design and modernization'):page.startsWith('read-')?works.find(w=>'read-'+w.id===page).title[lang]:page.startsWith('work-')?works.find(w=>'work-'+w.id===page).title[lang]:page==='missing'?'404':nav.find(n=>n[1]===active)[2];
  const html=`<!doctype html><html lang="${lang==='zh'?'zh-Hans':'en'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="${t('Tin-Yeh Huang 的个人网站：学习、研究、工程实践与公共服务。','Tin-Yeh Huang: studies, research, engineering, and public service.')} "><meta name="theme-color" content="#f5f5f7"><title>${h(title)}${page==='home'?'':' · Tin-Yeh Huang'}</title><link rel="stylesheet" href="${prefix}assets/leaflet/leaflet.css?v=${cacheKey}"><link rel="stylesheet" href="${prefix}site.css?v=${cacheKey}"><script defer src="${prefix}assets/gsap/gsap.min.js"></script><script defer src="${prefix}assets/gsap/ScrollTrigger.min.js"></script><script defer src="${prefix}assets/leaflet/leaflet.js"></script><script type="module" src="${prefix}site.mjs?v=${cacheKey}"></script></head><body data-page="${active}" data-lang="${lang}"><script>try{const theme=localStorage.getItem('tyh-theme')||(matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light');document.body.dataset.theme=theme;document.body.dataset.largeText=localStorage.getItem('tyh-large-text')||'false';document.querySelector('meta[name="theme-color"]').content=theme==='dark'?'#10151c':'#f5f5f7'}catch{document.body.dataset.theme='light'}</script><a href="#main" class="skip-link">${t('跳到内容','Skip to content')}</a><header class="site-header"><a href="index.html" class="brand">TYH<span>Tin-Yeh Huang</span></a><nav class="desktop-nav" aria-label="${t('主导航','Main navigation')}">${nav.map(([url,n,txt])=>`<a href="${url}" ${n===active?'aria-current="page"':''}>${txt}</a>`).join('')}${contactButton}</nav><div class="header-actions"><button data-theme-toggle aria-label="${t('切换明暗主题','Toggle light and dark theme')}">${icon('sun')}</button>${a(lang==='zh'?'en/'+file:'../'+file,lang==='zh'?'EN':'中文','language-link')}<button data-menu-toggle aria-expanded="false" aria-controls="site-menu" aria-label="${t('打开目录','Open menu')}">${icon('menu')}</button></div></header><dialog class="site-menu" id="site-menu"><header><span>Tin-Yeh Huang</span><button data-menu-close aria-label="${t('关闭目录','Close menu')}">${icon('x')}</button></header><nav>${nav.map(([url,n,txt],i)=>a(url,`<small>0${i+1}</small>${txt}${icon('chevron-right')}`,n===active?'active':'')).join('')}${contactButton}</nav></dialog>${contactPanel}<main id="main" tabindex="-1">${views[page]()}</main><footer class="site-footer"><div><span>© 2026 Tin-Yeh Huang</span>${contactButton}<a class="footer-email" href="mailto:tin-yeh.huang@connect.polyu.hk">${t('邮箱','Email')}${icon('arrow-up-right')}</a><button data-text-size aria-label="${t('调整文字大小','Adjust text size')}" aria-pressed="false">Aa</button></div></footer><aside class="music-dock" aria-label="${t('音乐播放器','Music player')}"><button class="music-launcher" popovertarget="music-panel" aria-label="${t('打开音乐播放器','Open music player')}"><span class="music-bars" aria-hidden="true"><i></i><i></i><i></i><i></i></span><span data-music-label>${t('听点音乐','A little music')}</span>${icon('plus')}</button><section id="music-panel" class="music-panel" popover aria-labelledby="music-title"><header><span class="eyebrow">${t('音乐角落','A little music')}</span><button popovertarget="music-panel" popovertargetaction="hide" aria-label="${t('收起播放器','Close music player')}">${icon('minus')}</button></header><div class="music-track"><span class="record-disc" aria-hidden="true"></span><div><h2 id="music-title">${h(music.title)}</h2><p>${h(music.artist)}</p><a href="${h(music.sourceUrl)}" target="_blank" rel="noopener noreferrer">Apple Music${icon('arrow-up-right')}</a></div></div><div class="music-transport"><button data-music-toggle aria-pressed="false" aria-label="${t('播放音乐试听','Play music preview')}">${icon('play')}</button><div class="music-timeline"><input type="range" data-music-seek min="0" max="30" value="0" step="0.1" disabled aria-label="${t('播放进度','Playback position')}"><div><time data-music-elapsed>0:00</time><span>${t('官方试听','Official preview')} · <time data-music-duration>0:30</time></span></div></div></div><label class="music-volume"><span>${t('音量','Volume')}</span><input type="range" data-music-volume min="0" max="1" step="0.05" value="0.55" aria-label="${t('音量','Volume')}"></label><p data-audio-status role="status"></p></section></aside><audio id="music" preload="none" src="${h(music.src)}"></audio></body></html>`;
  await writeFile(join(dest,file),html);
 }
 const redirects=[['writing.html','about.html#cabinet-title'],['research.html','collection.html?filter=papers'],['projects.html','collection.html?filter=projects'],['experience.html','about.html#experience'],['contact.html','about.html?contact=open'],...works.filter(w=>w.pdf||w.kind!=='paper').map(w=>[`work-${w.id}.html`,w.pdf?'read-'+w.id+'.html':'project-ninetoothed.html']),...experiences.map(e=>[`experience-${e.id}.html`,`about.html#record-${e.id}`])];
...
console.log('Built bilingual personal site: Home, separated Work categories, Notes, About, practice pages, publication records, and original-PDF readers.');
