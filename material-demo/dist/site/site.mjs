...
function syncSizeButton(){const large=document.documentElement.dataset.largeText==='true';sizeButton.setAttribute('aria-pressed',String(large));sizeButton.setAttribute('aria-label',large?t('使用较小字号','Use smaller text'):t('使用较大字号','Use larger text'));sizeButton.textContent=large?'A−':'A+';}
syncSizeButton();
...
music.addEventListener('playing',()=>{audioStatus.textContent=t('约 30 秒官方试听','~30-second official preview');});
music.addEventListener('waiting',()=>{audioStatus.textContent=t('正在缓冲试听…','Buffering preview…');});
...
function initialiseView({scrollToHash=true}={}){
 ...
 for(const viewport of [main.querySelector('[data-citymap]')]){
  ...
  const overview=()=>{map.setView([25,75],2,{animate:false});writePanel(zh?'北京 · 深圳 · 香港':'Beijing · Shenzhen · Hong Kong',zh?'点击地区标记，进入详细地图。':'Select a region pin to open its detailed map.',zh?'地点总览':'Place overview');};
  overview();
  ...
  for(const o of orgs){
   ...
   m.on('mouseover',()=>{writePanel(name,zh?'点击圆点，查看相关经历。':'Click the pin to read the related experience.',zh?'机构':'Institution');main.querySelector(`[data-org="${CSS.escape(o.id)}"]`)?.classList.add('is-map-hovered');});
  ...
  const regionPins=[
   ...
   {id:'bay',name:zh?'粤港澳大湾区':'Greater Bay Area',ll:[22.75,113.6],color:dotColor.hk}
  ].map(r=>{
   ...
   marker.on('mouseover',()=>writePanel(r.name,r.id==='bay'?(zh?'香港与深圳的机构':'Institutions in Hong Kong and Shenzhen'):(zh?'北京的机构':'Institutions in Beijing'),zh?'地区':'Region'));
  ...
  const updatePanel=()=>{
   ...
   if(selectedOrg){
    ...
    writePanel(selectedOrg.dataset.orgName,roles.join(' · '),zh?'机构与经历':'Institution & experience');
    return;
   }
   if(!currentCity){writePanel(zh?'北京 · 深圳 · 香港':'Beijing · Shenzhen · Hong Kong',zh?'点击地区标记，进入详细地图。':'Select a region pin to open its detailed map.',zh?'地点总览':'Place overview');return;}
   ...
   writePanel(title,names.join(' · '),zh?'相关机构':'Institutions');
  };
